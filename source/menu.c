...
void menu_disable(menu_t *menu, bool disable) { menu->disabled = disable; }
...
void menu_add_option(menu_t *menu, const char *option,
...
void menu_insert_option(menu_t *menu, u8 index, const char *label,
                        void (*callback)(game_t *)) {
  if (menu->num_options == 10) {
    return;
  }
...
void menu_remove_option(menu_t *menu, uint8_t index) {
  if (index < menu->num_options) {
    ...
    for (uint8_t i = index; i < menu->num_options - 1;
         i++) { // move entries arround
      menu->labels[i] = menu->labels[i + 1];
      menu->callbacks[i] = menu->callbacks[i + 1];
    }
    menu->labels[menu->num_options - 1] = NULL; // Clear last option
    menu->callbacks[menu->num_options - 1] =
        NULL; // Clear callback for removed option
    menu->num_options--;
  }
}

void menu_update_option(menu_t *menu, uint8_t index, const char *new_option,
                        void (*callback)(game_t *)) {
  if (index < menu->num_options) {
