...
void game_set_name(game_t *game) {
  ...
  if (utf16_to_utf8(utf8, utf8_len, (char16_t *)PersonalData->name, bytes_in) >
      0) {
...
void setup_settings_menu(game_t *game) {
  menu_init(game, setup_main_menu);
  menu_add_option(&game->menu,
                  game->is_sound_on ? "Sound: [ON]/OFF" : "Sound: ON/[OFF]",
                  game_sound_toggle);
  menu_add_option(&game->menu, "Back to Main Menu", setup_main_menu);
}
...
void game_sound_toggle(game_t *game) {
  ...
  menu_update_option(&game->menu, 0,
                     game->is_sound_on ? "Sound: [ON]/OFF" : "Sound: ON/[OFF]",
                     game_sound_toggle);
}

void game_debug_test(game_t *game) { game_init_boards(game); }

void game_connect(game_t *game) { nifi_connect(game); }
